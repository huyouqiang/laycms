...
$(function(){
    ...
    $('#groupForm').on('submit', function(e){
        e.preventDefault();
        var gid = $('#groupId').val();
        var data = { _token: '{{ csrf_token() }}', _method: gid ? 'PUT' : 'POST', name: $('#groupName').val(), sort_order: $('#groupSortOrder').val() || 0 };
        $.post(gid ? '/form-groups/'+gid : '{{ route("form-groups.store") }}', data).done(function(){ location.reload(); }).fail(function(x){ alert(x.responseJSON?.msg || '保存失败'); });
    });
});
</script>
@endpush
