...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('table-data.create', $form->table_name) }}" class="btn btn-sm btn-primary"><i class="bi bi-plus-lg me-1"></i>新增</a>
            @endif
        </div>
    </div>
    ...
    function loadData(){
        $.get(url, { page: page, limit: limit, search: $('#searchInput').val() }, function(res){
            ...
            (res.data || []).forEach(function(row, i){
                ...
                if(canDel) actions.append($('<button class="btn btn-sm btn-outline-danger"></button>').html('<i class="bi bi-trash me-1"></i>删除').on('click', function(){ if(confirm('确定删除？')) $.post(url+'/'+row.id, {_token:'{{ csrf_token() }}',_method:'DELETE'}, function(){ loadData(); }); }));
                tr.append(actions);
                tbody.append(tr);
            });
            total = res.count || 0;
