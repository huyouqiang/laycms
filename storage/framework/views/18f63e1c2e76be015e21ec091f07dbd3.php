        ...
        userModalIndex = layer.open({
            ...
            success: function(layero, index){
                ...
                layero.find('#userForm').on('submit', function(e){
                    ...
                    $.post(uid ? '/users/'+uid : '<?php echo e(route("users.store")); ?>', data).done(function(){ layer.close(index); location.reload(); }).fail(function(x){ layer.msg(x.responseJSON && x.responseJSON.msg ? x.responseJSON.msg : (x.responseJSON && x.responseJSON.errors ? JSON.stringify(x.responseJSON.errors) : '保存失败')); });
                });
            }
        });
    }
